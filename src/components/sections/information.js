    ...
    return (
     <div class="panel z-10 px-0 pt-32 bg-white t-0 overflow-hidden relative min-h-screen md:h-auto sm:h-auto">
      <div class="panel-body min-h-screen  sm:min-h-auto xs:min-h-auto h-100 w-90 lg:w-90 -mt-24 lg:-mt-32 md:-mt-3  xs:mt-5 lg:mt-3 lg:pt-3 p-0 sm:mt-5 mx-auto md:w-90 xs:w-100 sm:w-95 xl:flex md:flex  lg:flex sm:flex relative">
        ...
        <h4 class="text-center text-2xl pb-24 font-bold text-primary font-secondary">{this.props.title}</h4>
            ...
            <div class="pt-4 px-5 relative">
              <ul class="list text-center w-100 lg:w-60  font-primary mr-auto p-0 m-0 hidden xl:block lg:block px-0 mr-0  text-sm">
                {this.props.cards.map((item, index) =>  <li key={index} onClick={()=> this.switchCard(index)} className={['inline-block my-0 py-1 mr-8 text-3xl text-grey font-bold cursor-pointer', this.state.index == index ? 'text-primary border-0 border-b-4 border-solid border-grey' : null].join(' ') }>{index + 1}.</li>)}
              </ul>
            </div>
            ...
            <div class="w-50 px-4 text-right">
              <button onClick={()=> this.slideLeft()} class="btn  bg-transparent hover:bg-primary hover:text-white cursor-pointer text-primary py-1 px-4 border-primary border-6 text-2xl">
                  <FontAwesomeIcon icon="arrow-left" />
              </button>
            </div>
            <div class="w-50 px-4 text-left">
              <button onClick={()=> this.slideRight()} class="btn  bg-transparent hover:bg-primary hover:text-white cursor-pointer text-primary py-1 px-4 border-primary border-6 text-2xl">
...
Information.defaultProps = {
  ...
  cards: [
    {
      text: '<p>Discover where your leads come from, what it costs to get them, and how they interact with your websitebefore contacting you.</p>'
    },
    {
      text: '<p>We do software testing for flawless products. With a purpose.</p>'
    },
    {
      text: '<p>Our expert testers handle quality assurance so you can focus on building your ideas.</p>'
    }
  ]
}
